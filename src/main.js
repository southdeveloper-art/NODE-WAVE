...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function initDDoS() {
    ...
    if (trafficCanvas) {
      ...
      function updateData() {
        // 1. Clean Traffic: High-velocity jitter
        const cleanBase = 35 + 8 * Math.sin(t * 0.08); // Significantly faster waves
        const cleanNoise = (Math.random() - 0.5) * 18; // Sharp professional jitter
        cleanData.push(Math.max(12, cleanBase + cleanNoise));
        if (cleanData.length > historyN) cleanData.shift();

        // 2. Malicious Traffic: Hyper-speed Attacks
        if (!attackActive && Math.random() < 0.02) {
          attackActive = true;
          attackTimer = 0;
          attackPeak = 50 + Math.random() * 45;
        }
        ...
        if (attackActive) {
          ...
          let val = 0;
          if (attackTimer < 4) {
            val = (attackTimer / 4) * attackPeak; // Clinical fast ramp
          } else {
            val = attackPeak * Math.exp(-(attackTimer - 4) * 0.18); // Rapid technical decay
          }

          val += (Math.random() - 0.5) * 25; // High-fidelity noise
          malData.push(Math.max(0, val));

          if (attackTimer > 35 && val < 2) {
            attackActive = false;
          }
        } else {
          malData.push(Math.random() * 4);
        }
